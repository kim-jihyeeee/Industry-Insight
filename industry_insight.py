...
# 1. 페이지 설정
st.set_page_config(page_title="AE Total Tool v14.6", layout="wide")
...
def get_failsafe_keywords(titles):
    ...
    try:
        resp = ai_engine.generate_content(f"{titles}에서 핵심 키워드 5개만 #단어로 뽑아줘. 이슈, 트렌드 단어 제외.")
...
if main_menu == "🌐 AI Trend Radar":
    ...
    with t2:
        ...
        s_period = c2.selectbox("수집 기간", ["3일", "7일", "한달", "60일", "분기"], index=2, key="s_p")
        if st.button("🔍 검색 AI 분석 시작"):
            ...
            if titles:
                tags = get_failsafe_keywords(titles)
                st.markdown("<div class='section-header'>🎯 대중 관심사 키워드</div>", unsafe_allow_html=True)
                st.markdown("".join([f"<span class='issue-tag'>{t}</span>" for t in tags]), unsafe_allow_html=True)
                fig, _ = create_cleaned_wc(titles)
                st.pyplot(fig)

# 2. 광고주 DB 관리 (백업 기능 유지)
...
# 3. 관리 이력 입력 (🌟 검색 로직 수정)
    ...
    with st.form("input_form", clear_on_submit=True):
        ...
        # 🌟 업체명 일부 입력 시 검색 로직
        in_name_text = st.text_input("새 광고주명 입력 (일부 입력 시 기존 업체 검색)")
...
# 4. 내부 소통 이슈 리포트 (🌟 TypeError 해결)
    ...
    if not db.empty:
        ...
        # 🌟 [해결] Datetime 형식을 Date 형식으로 변환하여 비교
        ...
        if not f_df.empty:
            ...
            if fig:
                st.pyplot(fig)
                st.download_button("🖼️ 이미지 저장", buf.getvalue(), f"{target}_WC.png")
        ...
        if st.button(f"🔗 {s_cat} 트렌드 불러오기"):
            ...
            if fig_t:
                st.pyplot(fig_t)
                st.download_button("🖼️ 시장 이미지 저장", buf_t.getvalue(), f"{s_cat}_Trend.png")
