...
# 1. 페이지 설정
st.set_page_config(page_title="AE Total Tool v15.0", layout="wide")
...
# 3. 관리 이력 입력 (스마트 검색 필터 적용)
    ...
    # 🌟 필터링 검색 바
    search_q = st.text_input("🔍 광고주 검색 필터", placeholder="'그린' 등 업체명 일부를 입력하면 아래 목록이 바뀝니다.")
    ...
    with st.form("input_form", clear_on_submit=True):
        ...
        in_name_text = st.text_input("새 광고주명 입력 (위 목록에 없을 때만)")
        
        c3, c4 = st.columns(2)
        in_m_cat = c3.selectbox("대분류 카테고리", list(DETAILED_CATEGORIES.keys()))
        in_s_cat = c4.selectbox("세부 분류 선택", DETAILED_CATEGORIES[in_m_cat])
        in_content = st.text_area("소통 내용 입력")
...
# 4. 내부 소통 이슈 리포트 (날짜 에러 해결 및 상하 배치)
    ...
    if not db.empty and '광고주명' in db.columns:
        ...
        # 광고주 검색 선택
        target_search = c1.text_input("🔍 분석 대상 광고주 검색", placeholder="업체명 입력")
        target_list = [c for c in all_clients if target_search in c] if target_search else all_clients
        target = c1.selectbox("최종 분석 대상 선택", target_list)
        ...
        # 상단: 내부 소통 이슈
        st.markdown(f"<div class='section-header'>💬 {target} 내부 소통 이슈 리포트</div>", unsafe_allow_html=True)
        f_df = db[db['광고주명'] == target].copy()
        
        # 🌟 [에러 해결] 날짜 형식 강제 변환 및 비교
        ...
        if not f_df.empty:
            ...
            if fig1: 
                st.pyplot(fig1)
                st.download_button("🖼️ 리포트 이미지 저장", buf1.getvalue(), f"{target}_소통.png", key="d1")
        
        # 하단: 시장 트렌드 매칭
        st.markdown(f"<div class='section-header'>🌏 {s_cat} 시장 트렌드 매칭 분석</div>", unsafe_allow_html=True)
        if st.button(f"🔗 실시간 {s_cat} 시장 트렌드 불러오기"):
            ...
            if fig2: 
                st.pyplot(fig2)
                st.download_button("🖼️ 트렌드 이미지 저장", buf2.getvalue(), f"{s_cat}_Trend.png", key="d2")
    else:
        st.info("📂 광고주 DB를 먼저 업로드해 주세요.")
