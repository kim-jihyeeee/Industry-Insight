...
st.set_page_config(page_title="AE Total Tool v22.8", layout="wide")
...
st.sidebar.title("🚀 AE Total Tool v22.8")
menu = st.sidebar.radio("메뉴", ["🌐 AI Trend Radar", "📂 DB 관리", "📝 이력 입력", "📊 리포트"])
...
if menu == "🌐 AI Trend Radar":
    ...
    for i, tab in enumerate([t1, t2]):
        with tab:
            ...
            p = c2.selectbox("기간 설정", ["3일", "7일", "한달", "60일", "분기"], index=2, key=f"p{i}") # 🌟 기간 설정 복구
